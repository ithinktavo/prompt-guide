#!/usr/bin/env python3
"""
Generate a standalone PDF of the 12 Prompt Engineering Techniques Workbook.

This is the follow-along workbook for the Priority Builder talk. Teaches
the 12 techniques in depth with exercises, excerpts from Joey's file, and
an audit checklist.
"""
...
SOURCE_FILE = os.path.join(PROJECT_DIR, "prompt-techniques-workbook.md")
OUTPUT_FILE = os.path.join(PROJECT_DIR, "prompt-techniques-workbook.pdf")
...
def on_content_page(canvas, doc):
    ...
    canvas.drawString(inch, 0.5 * inch, "The 12 Prompt Engineering Techniques \u2014 Workbook")
...
def build_cover(styles, avail_width):
    ...
    flows.append(Paragraph(
        "<font color='#26a69a'>━━━</font>  <font color='#3f51b5'>WORKBOOK</font>  <font color='#c62828'>━━━</font>",
    ...
    flows.append(Paragraph("The 12 Prompt", styles['CoverTitle']))
    flows.append(Paragraph("Engineering Techniques", styles['CoverTitle']))
    flows.append(Spacer(1, 12))
    flows.append(HR(avail_width * 0.3, INDIGO_500, 2))
    flows.append(Spacer(1, 18))

    flows.append(Paragraph(
        "A Complete Follow-Along Workbook<br/>for the Priority Builder Talk",
        styles['CoverSubtitle']
    ))
    flows.append(Spacer(1, 30))

    flows.append(Paragraph(
        "Twelve techniques. Twelve exercises. One audit checklist you'll use on every prompt you write.",
        styles['CoverDesc']
    ))
    ...
    flows.append(Paragraph("12 Techniques  \u2022  12 Exercises  \u2022  ~2 hours", info))
    flows.append(Spacer(1, 30))
...
def build_about_page(styles, avail_width):
    ...
    flows.append(Paragraph(
        "A follow-along companion to the talk <i>\u201cFrom Prompt to Template: The "
        "Priority Builder Case Study.\u201d</i> It walks through each of the 12 prompt "
        "engineering techniques demonstrated in the talk, with excerpts from "
        "Joey's actual prompt file, a 5-minute exercise for each technique, and "
        "an audit checklist you can use on any prompt of your own.",
        body
    ))
    flows.append(Spacer(1, 8))
    flows.append(Paragraph(
        "Estimated time: ~2 hours if you do every exercise. ~45 minutes if you read through. "
        "Each technique chapter is self-contained \u2014 skip around as needed.",
        body
    ))
    flows.append(Spacer(1, 8))
    flows.append(Paragraph(
        "Best read <b>after attending the talk</b> or reading the talk deck. "
        "If you haven't, start with Chapter 1 \u2014 it sets up the case study in 2 minutes.",
        body
    ))
    ...
    flows.append(Paragraph(
        ...
        "with the help of AI (Anthropic's Claude). The techniques it teaches are "
        "observed in a real prompt file shared internally by our colleague Joey, "
        "whose FY26 Priority Builder is the running case study throughout.",
        body
    ))
...
def main():
    ...
    doc = WorkbookDocTemplate(
        ...
        title="The 12 Prompt Engineering Techniques \u2014 Workbook",
        author=AUTHOR_NAME,
